...
func CreateNewBlock(w http.ResponseWriter, req *http.Request) {
	...
	Res, err := http.Post("http://192.168.103.193:5000/Apply/Career", "application/json", bytes.NewBuffer(bodyTwo)) // middle에서 받은 데이터 GenTx한테 주고 Txid 받기
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	hi, errr := http.Post("http://192.168.103.117:3000/VerifySign", "application/json", bytes.NewBuffer(byebye))
	if errr != nil {
		fmt.Println(errr)
	}
	Che := new(Che)
